...
const curriculum = window.MATH_CURRICULUM || null;
...
function renderMathHome() {
  ...
  homeBtn.hidden = true;
  const semesterBlocks = curriculum.semesters.map(semester => {
    const units = semester.unitIds
      .map(unitId => curriculum.units.find(unit => unit.id === unitId))
      .filter(unit => unit && unit.enabled);
    return `<section class="semester-block semester-${semester.id}" aria-labelledby="semester-${semester.id}-title">
      <div class="semester-heading">
        <div><p class="semester-label">學期課程</p><h2 id="semester-${semester.id}-title">${semester.title}</h2></div>
        <p>${semester.description}</p>
      </div>
      <div class="unit-grid semester-unit-grid">
        ${units.map(unit => `<button class="unit-card unit-${unit.color}" data-unit-id="${unit.id}">
  ...
  card.innerHTML = `<div class="platform-home">
    ...
    <h1>今天要學什麼？</h1>
    <p class="instruction">請先選擇學期，再選擇數學單元。</p>
    <div class="semester-list">${semesterBlocks}</div>
  </div>`;
...
function renderLoadError() {
  ...
  lessonActions.hidden = true;
  homeBtn.hidden = true;
  card.innerHTML = `<div class="platform-home load-error" role="alert">
    <p class="eyebrow">數學學習站</p>
    <h1>課程資料正在重新整理</h1>
    <p class="instruction">首頁資料尚未完成載入，請按下面的按鈕再試一次。</p>
    <button class="primary-button" id="reloadHomeBtn" type="button">重新載入首頁</button>
  </div>`;
  card.querySelector("#reloadHomeBtn").addEventListener("click", () => window.location.reload());
}
